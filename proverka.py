...
async def start(message: types.Message, state: FSMContext):
    await state.finish()  # очистка состояний
...
async def check_prompt(callback_query: types.CallbackQuery, state: FSMContext):
...
async def admin_blacklist_add_prompt(callback_query: types.CallbackQuery, state: FSMContext):
    if callback_query.from_user.id not in ADMIN_IDS:
        return
    await BlacklistAddForm.waiting_for_username.set()
...
@dp.message_handler(state=BlacklistAddForm.waiting_for_username)
...
async def admin_blacklist_remove_prompt(callback_query: types.CallbackQuery, state: FSMContext):
    if callback_query.from_user.id not in ADMIN_IDS:
        return
    await BlacklistRemoveForm.waiting_for_username.set()
    await callback_query.message.edit_text("🗑 *Введите username канала для удаления из чёрного списка:*", parse_mode="Markdown")
    await callback_query.answer()

@dp.message_handler(state=BlacklistRemoveForm.waiting_for_username)
...
async def admin_monitor_add_prompt(callback_query: types.CallbackQuery, state: FSMContext):
    if callback_query.from_user.id not in ADMIN_IDS:
        return
    await MonitorAddForm.waiting_for_username.set()
    await callback_query.message.edit_text("📋 *Введите username канала для мониторинга:*\nПример: @durov", parse_mode="Markdown")
    await callback_query.answer()

@dp.message_handler(state=MonitorAddForm.waiting_for_username)
...
async def admin_monitor_remove_prompt(callback_query: types.CallbackQuery, state: FSMContext):
    if callback_query.from_user.id not in ADMIN_IDS:
        return
    await MonitorRemoveForm.waiting_for_username.set()
    await callback_query.message.edit_text("🗑 *Введите username канала для удаления из мониторинга:*", parse_mode="Markdown")
    await callback_query.answer()

@dp.message_handler(state=MonitorRemoveForm.waiting_for_username)
...
async def main():
    ...
    logger.info("Бот проверки каналов запущен (aiogram 2.25.1)")
    # Запускаем polling
