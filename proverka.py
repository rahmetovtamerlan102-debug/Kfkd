...
def get_top_risky(limit=10):
    ...
    rows = cur.fetchall()
    conn.close()
    # Группируем по username, берём последний результат
    latest = {}
    for row in rows:
...
@dp.message_handler(lambda m: hasattr(m, 'state') and m.state == BlacklistForm.waiting_for_username.state)
...
@dp.message_handler(lambda m: hasattr(m, 'state') and m.state == MonitorForm.waiting_for_username.state)
