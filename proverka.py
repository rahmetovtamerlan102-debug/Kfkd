...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS channel_blacklist (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        username TEXT UNIQUE,
        reason TEXT,
        added_by INTEGER,
...
def save_scan_log(username, risk_score, risk_level, details=""):
    conn = sqlite3.connect(DB_NAME)
    conn.execute("INSERT INTO scan_logs (username, risk_score, risk_level, details) VALUES (?,?,?,?)",
...
def get_stats():
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    total = cur.execute("SELECT COUNT(*) FROM scan_logs").fetchone()[0]
    high = cur.execute("SELECT COUNT(*) FROM scan_logs WHERE risk_level='high'").fetchone()[0]
    medium = cur.execute("SELECT COUNT(*) FROM scan_logs WHERE risk_level='medium'").fetchone()[0]
    low = cur.execute("SELECT COUNT(*) FROM scan_logs WHERE risk_level='low'").fetchone()[0]
    blacklisted = cur.execute("SELECT COUNT(*) FROM channel_blacklist").fetchone()[0]
    conn.close()
...
def get_top_risky(limit=10):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    rows = cur.execute("SELECT DISTINCT username, risk_score, risk_level, scanned_at FROM scan_logs ORDER BY scanned_at DESC").fetchall()
    conn.close()
    latest = {}
    for row in rows:
...
def get_monitored_channels():
    conn = sqlite3.connect(DB_NAME)
    rows = conn.execute("SELECT username, added_at FROM monitored_channels ORDER BY added_at").fetchall()
    conn.close()
    return rows
...
def get_blacklist():
    conn = sqlite3.connect(DB_NAME)
    rows = conn.execute("SELECT username, reason, added_at FROM channel_blacklist ORDER BY added_at").fetchall()
    conn.close()
    return rows
...
def is_blacklisted(username):
    conn = sqlite3.connect(DB_NAME)
    res = conn.execute("SELECT 1 FROM channel_blacklist WHERE username=?", (username,)).fetchone()
    conn.close()
...
def extract_username_from_input(text):
    text = text.strip()
    # Ссылки t.me/username, telegram.me/username
    match = re.search(r'(?:https?://)?(?:www\.)?(?:t\.me|telegram\.me)/([a-zA-Z0-9_]+)', text)
    if match:
        return match.group(1)
    # @username или просто username (минимум 5 символов, можно изменить)
...
def check_prompt(call):
    msg = bot.send_message(call.message.chat.id, "🔍 *Введите ссылку или username канала:*\nПример: @durov или https://t.me/durov",
                           parse_mode="Markdown")
...
def process_check(message, chat_id, original_msg_id):
    ...
    if not username:
        bot.send_message(chat_id, "❌ Не удалось распознать канал. Попробуйте ещё раз: @username или https://t.me/username",
                         reply_markup=main_kb())
        return
    bot.send_message(chat_id, f"🔍 Проверяю канал: @{username}...")
    result = check_channel(username)
    if "error" in result:
        bot.send_message(chat_id, f"❌ Ошибка: {result['error']}", reply_markup=main_kb())
        return
