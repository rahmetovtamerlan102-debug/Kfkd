...
class MonitorAddForm(StatesGroup):
    waiting_for_username = State()

class MonitorRemoveForm(StatesGroup):
    waiting_for_username = State()

class BlacklistAddForm(StatesGroup):
    waiting_for_username = State()

class BlacklistRemoveForm(StatesGroup):
    waiting_for_username = State()
...
async def check_prompt(callback: CallbackQuery, state: FSMContext):
    await state.set_state(CheckForm.waiting_for_username)
    await callback.message.edit_text("🔍 *Введите username канала:*\nПример: @durov", parse_mode="Markdown")
    await callback.answer()
...
async def top_risky(callback: CallbackQuery):
    rows = get_top_risky(10)
    if not rows:
        await callback.message.edit_text("📭 Нет данных для рейтинга", reply_markup=back_kb())
        await callback.answer()
        return
...
async def admin_panel(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        ...
        return
    await callback.message.edit_text("🔐 *Админ панель*", parse_mode="Markdown", reply_markup=admin_kb())
    await callback.answer()
...
async def admin_blacklist_menu(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        return
    await callback.message.edit_text("🚫 *Управление чёрным списком*", parse_mode="Markdown", reply_markup=blacklist_kb())
    await callback.answer()

@dp.callback_query(F.data == "admin_blacklist_add")
async def admin_blacklist_add_prompt(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        return
    await state.set_state(BlacklistAddForm.waiting_for_username)
    await callback.message.edit_text("🚫 *Введите username канала для добавления в чёрный список:*\nПример: @scam_channel", parse_mode="Markdown")
    await callback.answer()

@dp.message(StateFilter(BlacklistAddForm.waiting_for_username))
...
async def admin_blacklist_list(callback: CallbackQuery):
    ...
    rows = get_blacklist()
    if not rows:
        await callback.message.edit_text("📭 Чёрный список пуст", reply_markup=blacklist_kb())
        return
    text = "🚫 *Чёрный список каналов:*\n\n"
    for r in rows:
        text += f"├ @{r[0]} (добавлен {r[2][:10]})\n"
    await callback.message.edit_text(text, parse_mode="Markdown", reply_markup=blacklist_kb())
    await callback.answer()

@dp.callback_query(F.data == "admin_blacklist_remove")
async def admin_blacklist_remove_prompt(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        return
    await state.set_state(BlacklistRemoveForm.waiting_for_username)
    await callback.message.edit_text("🗑 *Введите username канала для удаления из чёрного списка:*", parse_mode="Markdown")
    await callback.answer()

@dp.message(StateFilter(BlacklistRemoveForm.waiting_for_username))
...
async def admin_monitor_add_prompt(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        return
    await state.set_state(MonitorAddForm.waiting_for_username)
    await callback.message.edit_text("📋 *Введите username канала для мониторинга:*\nПример: @durov", parse_mode="Markdown")
    await callback.answer()

@dp.message(StateFilter(MonitorAddForm.waiting_for_username))
...
async def admin_monitor_list(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        return
    rows = get_monitored_channels()
    if not rows:
        await callback.message.edit_text("📭 Список мониторинга пуст", reply_markup=admin_kb())
        return
...
async def admin_monitor_remove_prompt(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        return
    await state.set_state(MonitorRemoveForm.waiting_for_username)
    await callback.message.edit_text("🗑 *Введите username канала для удаления из мониторинга:*", parse_mode="Markdown")
    await callback.answer()

@dp.message(StateFilter(MonitorRemoveForm.waiting_for_username))
...
async def main():
    ...
    logger.info("Бот проверки каналов запущен (aiogram 3, Python 3.11)")
